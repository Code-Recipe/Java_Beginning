...
运算和操作符
-----
运算符又叫做操作符，分为算术运算符、关系运算符、逻辑运算符和赋值运算符四种。其中三种我们都已经讲解过了，在这里就给大家讲解一下算术运算符：

### 算术运算符
算术运算符就和数学中我们用来计算的符号大抵相同，在此不作赘述。贴上常见算数运算符：
![算术运算符](Pic2.png)
TIPS:上面这些运算符大家应该都很熟悉吧。然而大家可能会奇怪为什么10/4返回2。因为10是整数，4也是整数，那么计算机就会认为这个运算是整数运算，因此返回的结果也是整数结果。这个比较可能考，需要注意，凡是几个整数进行运算都会默认使用整数运算，如果要让程序进行小数运算，需要把整数转换为小数才可以。（比如，10.0/4就会输出2.5。原因就是计算机把10.0识别为了浮点数，进行了浮点运算）
...
这些看起来很复杂，实际上是比较符合我们的数学常识的。大家可以不用花太大心思去记忆，不确定时来查即可。
